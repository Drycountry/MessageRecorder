...
constexpr std::size_t kInvalidSegmentListIndex =
...
bool IsValidReplaySpeed(double speed) {
...
bool SegmentMightContainRecordSequence(const SegmentSummary& segment, std::uint64_t value) {
  return segment.record_count != 0 &&
         (!segment.last_record_seq.has_value() || *segment.last_record_seq >= value);
}
...
bool FindSegmentListIndex(const internal::ManifestData& manifest,
                          const ReplayCursor& cursor,
                          std::size_t* segment_list_index,
                          std::string* error) {
  switch (cursor.kind) {
...
bool FindRecordIndexInSegment(const std::vector<ReplayMessage>& records,
                              const ReplayCursor& cursor,
  ...
  switch (cursor.kind) {
    case ReplayCursorKind::kRecordSequence: {
      const auto it = std::lower_bound(
          ...
          [](const ReplayMessage& record, std::uint64_t value) {
            return record.record_seq < value;
          });
      if (it == records.end()) {
        return false;
      }
    ...
    case ReplayCursorKind::kEventMonoTime: {
      const auto it = std::lower_bound(
          records.begin(), records.end(), cursor.value,
          [](const ReplayMessage& record, std::uint64_t value) {
            return record.event_mono_ts_us < value;
          });
      if (it == records.end()) {
        return false;
      }
    ...
    case ReplayCursorKind::kEventUtcTime: {
      const auto it = std::lower_bound(
          records.begin(), records.end(), cursor.value,
          [](const ReplayMessage& record, std::uint64_t value) {
            return record.event_utc_ts_us < value;
          });
      if (it == records.end()) {
        return false;
      }
...
class ReplaySession final : public IReplaySession {
 public:
  ReplaySession(std::filesystem::path recording_path,
 ...
 private:
  ...
  bool LoadSegmentRecordsLocked(std::size_t segment_list_index, std::string* error) {
    ...
    if (!internal::ScanSegment(recording_path_ / manifest_.segments[segment_list_index].file_name,
                               options, &scan_result, error)) {
      return false;
    }
  ...
  bool SeekToFirstRecordFromSegmentLocked(std::size_t start_segment_list_index,
                                          const std::string& eof_message,
                                          std::string* error) {
  ...
  bool SeekToLowerBoundLocked(std::size_t start_segment_list_index,
                              const ReplayCursor& cursor,
                              const std::string& eof_message,
                              std::string* error) {
    for (std::size_t index = start_segment_list_index; index < manifest_.segments.size(); ++index) {
  ...
  bool SeekLocked(const ReplayCursor& cursor, std::string* error) {
    ...
    switch (cursor.kind) {
      case ReplayCursorKind::kRecordSequence:
        return SeekToLowerBoundLocked(segment_list_index, cursor,
                                      "record sequence cursor is beyond end of recording",
                                      error);

      case ReplayCursorKind::kEventMonoTime:
        return SeekToLowerBoundLocked(segment_list_index, cursor,
                                      "monotonic time cursor is beyond end of recording", error);

      case ReplayCursorKind::kEventUtcTime:
        return SeekToLowerBoundLocked(segment_list_index, cursor,
                                      "UTC time cursor is beyond end of recording", error);

      case ReplayCursorKind::kSegmentCheckpoint:
        return SeekToFirstRecordFromSegmentLocked(segment_list_index,
                                                  "segment checkpoint is beyond end of recording",
                                                  error);
    }
  ...
  PrepareEntryStatus PrepareCurrentMessageLocked(ReplayMessage* message,
  ...
  bool WaitForPlaybackDelay(std::uint64_t logical_delta_us,
                            const std::chrono::steady_clock::time_point& reference_time,
    ...
    for (;;) {
      ...
      if (applied_speed > 0.0) {
        const double elapsed_wall_us =
            std::chrono::duration<double, std::micro>(now - wall_checkpoint).count();
      ...
      if (stop_requested_ || paused_ || timing_reset_requested_ ||
          cursor_generation_ != observed_generation) {
        return false;
      }

      const auto remaining_wall =
          std::chrono::duration<double, std::micro>(remaining_logical_us / current_speed);
      ...
      const auto sleep_duration =
          std::min(remaining_wall, std::chrono::duration<double, std::micro>(1000.0));
      cv_.wait_for(lock, sleep_duration, [&]() {
        return stop_requested_ || paused_ || timing_reset_requested_ ||
               cursor_generation_ != observed_generation || options_.speed != current_speed;
      });
    }
  }
    ...
    for (;;) {
      ...
      if (!target_->OnMessage(entry, &callback_error)) {
        ...
        result_.failure = ReplayFailure{entry.record_seq, entry.event_utc_ts_us,
                                        callback_error.empty() ? "replay target rejected message"
                                                              : callback_error};
        finished_ = true;
        return;
      }
...
std::unique_ptr<IReplaySession> CreateReplaySession(
    ...
    const ReplayOptions& options, std::unique_ptr<IReplayTarget> target, std::string* error) {
  if (target == nullptr) {
  ...
  auto session = std::make_unique<ReplaySession>(recording_path, std::move(manifest), options,
                                                 std::move(target));
